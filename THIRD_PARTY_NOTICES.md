...
This file records direct third-party dependencies and build-time source inputs used by this repository. It complements LICENSE, NOTICE, and GPL-COMPLIANCE.md.

Keep the upstream license text and notices with any redistributed binary artifact. For release audits, also verify transitive dependency licenses from the committed lock files and published package metadata.
...
non-kernel KernelSU-derived code is GPL-3.0-or-later unless an individual file says otherwise.
...
The committed website/yarn.lock pins transitive npm packages for the documentation build. Those packages are not vendored in this repository; their licenses must be taken from the npm package metadata at install/release time.
...
Gradle resolves additional transitive dependencies from the repositories above. For binary releases, generate a dependency report from Gradle and preserve all licenses/notices from resolved artifacts.

Suggested release check:

## Additional referenced open source projects
### Sukisu-Ultra
Repository: https://github.com/SukiSU-Ultra/SukiSU-Ultra
Purpose: Reference SuSFS file system hiding & hybrid mount implementation
License: Please refer to the license statement of the original repository

### FolkPatch
Repository: https://github.com/LyraVoid/FolkPatch
Purpose: Part of UI framework source code quoted in manager application
License: Apache License 2.0

### SuSFS
Purpose: Kernel layer file hiding, hybrid mount solution adapted for GKI kernel

### KPM (Kernel Patch Module)
Purpose: Dynamic kernel module patching mechanism, compatible with GKI LKM loading logic

> UI Statement:
> The MIUI-style visual theme implemented in this project is an independently developed visual scheme.
> No official MIUI source code or graphic resources are copied. There is no derivative relationship with Xiaomi official software.

> Internationalization Statement:
> All multi-language translation resources are independently maintained by this project.
> Welcome community contributors to submit translations to expand language support.
...
The repository does not vendor the DDK container. It is fetched by CI at build time and remains governed by its upstream licenses.

When publishing a KO package, include:
1. the exact ApkeSU source revision;
2. the exact KMI and DDK release;
3. the generated binary artifact and enough build instructions to reproduce it.
cd website
